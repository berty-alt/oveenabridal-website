...
export function AdminDashboard({ email }: { email: string }) {
  ...
  async function reviewRequest(id: string, status: RentalRequest["status"]) {
            ...
            <>
              ...
              <div className="request-list">
                ...
                {requests.map((item) => <article key={item.id}>
                  ...
                  <div className="request-actions"><a href={`https://wa.me/${item.phone.replace(/\D/g, "")}`} target="_blank" rel="noreferrer">WhatsApp customer</a><button onClick={() => reviewRequest(item.id, "rejected")}>Reject</button><button className="approve" onClick={() => reviewRequest(item.id, "approved")}>Approve</button></div>
                </article>)}
              </div>
            </>
          )}
